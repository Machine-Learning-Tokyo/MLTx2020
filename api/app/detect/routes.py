...
from app.detect import customerservices as cs
...
def detect():
    ...
    if request.method == "POST":
        for (key, f) in (request.files.items()):
            ...
            xs = ['Angry', 'Disgusted', 'Fearful', 'Happy', 'Neutral', 'Sad']
            ys = []
            data = emo_count
            ys = [data['Angry'], data['Disgusted'], data['Fearful'], data['Happy'], data['Neutral'], data['Sad']]
...
def result():
    ...
    return render_template('detect/results.html', file_name=file_name, people_count=person_count)

@bp.route('/heatmap')
def public():
    print("Heatmap...")
    return render_template('detect/public.html')
...
place_types = ['caffe', 'restaurant', 'sushi', 'nightclub', 'bar', 'toilet']
service = cs.customerservices()

@bp.route("/get_places", methods=["GET"])
def get_places():
   params = request.json
   if (params == None):
       params = request.args    
       if (params is None):
           return 'Error: Please insert type and coordinates!'   
   ...
   if coordinates is None:
       return 'Please enter your coordinates'
   ...
   msg = None
   msg = service.solve_request(camlist, place_type, coordinates)
   msg = msg.reset_index(drop=True)
   n_cols = {'nbpeople': 'Queue size', 'waiting': 'waiting time',
             'age': 'average age', 'gender': 'average gender',
             'emotion': 'average emotion', 'duration': 'eta'}
             
   msg = msg.rename(index=str, columns=n_cols)    
   args = dict()
   args.clear()
   args['place_type'] = place_type
   args['msg'] = msg
   print(args)
   return render_template('detect/index.html', **args)
